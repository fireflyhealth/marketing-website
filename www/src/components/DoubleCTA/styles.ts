export const Wrapper = [
  'relative w-full pt-8 p-4 pb-[127px]',
  'md:absolute md:top-auto md:p-12',
];

export const BackgroundColor = [
  'absolute top-0 left-0 z-navigationCTAGradient w-full h-full bg-yellow-linear-gradient backdrop-blur-[75px]',
];

export const CTA_Container = [
  'flex flex-col space-y-3',
  'md:flex-row md:space-y-0 md:space-x-12',
];

export const CTA_Card_Wrapper = ['w-full'];

export const CTA_Card = [
  'w-full z-navigationCTAContent bg-white p-5 flex flex-col space-y-3 rounded-[14px] focus:border-black focus:border-[3px] focus:outline-none',
  'md:pt-24 md:space-y-8',
];

export const Card_Label = ['font-roobert font-size-8'];

export const CTA = ['flex flex-row justify-between items-center'];

export const Label = ['font-trust font-size-3'];
