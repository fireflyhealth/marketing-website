...
import ContactPage, { ContactPageProps } from '@/pages/contact';

const ContactPagePreview: NextTypes.PageRoute<
  InferGetServerSidePropsType<typeof getServerSideProps>
> = ({ contactPage, siteSettings, previewToken }) => {
  const [livePage, setLivePage] = useState(contactPage);
  ...
  */
  ...
  return <ContactPage contactPage={livePage} siteSettings={liveSettings} />;
};

export const getServerSideProps: GetServerSideProps<
  ContactPageProps & { previewToken: string }
> = async (context) => {
  // @ts-ignore
  ...
  const contactPage = await Sanity.contactPage
    ...
    */
      ...
      if (contactPage === null)
        return new Promise<SanityTypes.ContactPage>((resolve) => {
          setTimeout(() => {
            Sanity.contactPage.fetchPreview(previewToken).then(resolve);
          }, 1000);
        });
